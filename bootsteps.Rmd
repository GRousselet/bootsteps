...
source("./functions/Rallfun-v35-light.txt") # simplified version to run the examples in this notebook only.
...
The previous chunk loads a simplified version of Rand Wilcox's collection of functions. To get all the functions described in his Robust Estimation and Hypothesis Testing book, do: `source("./functions/Rallfun-v35.txt")`. The latest version of `Rallfun` is available [online](http://dornsife.usc.edu/labs/rwilcox/software/). **If you use functions from the full file (`Rallfun-v35.txt`) or a more recent file, ensure to set the `SEED` argument to `FALSE`. For instance in `onesampb()` called later in this notebook: onesampb(x,est=median,alpha=.05,nboot=2000,SEED=FALSE). If `SEED=TRUE`, the function will always return the same bootstrap results -- see below for explanation about setting the seed of random number generators in *R*.**
...
Also, note that in *R*, like in *Python*, indexing is done using square brackets (e.g. boot.m[B]), whereas some other languages use parentheses (*Matlab* for instance).
...
ggplot(df, aes(x = x, y = y)) + theme_gar +
  ...
  geom_segment(aes(x=x, xend=x, y=0, yend=y)) + # comment to make a scatterplot instead of a lollipop chart
...
There are many ways to estimate quantiles. The R function `quantile()` has 9 options for instance, with default to `type=7`. Hesterberg (2015) recommends using `type=6`. In Rand Wilcox's functions, the procedure in the next chunk is used, which gives results slightly different from those in the previous chunk. With a large number of bootstrap samples, which quantile method is used will make little difference. 
...
Another option is to use the `boot()` function from the `boot` package (Canty & Ripley, 2017; Davison & Hinkley, 1997).
This function is more complicated than using functions with a narrow focus like `onesampb()` because it requires creating a function to process the data. The advantage is that the bootstrap distribution is saved and can be used in a second step to make illustrations and compute various quantities. You can find a tutorial about the `boot` package for instance [here](https://www.datacamp.com/community/tutorials/bootstrap-r), [here](https://www.r-bloggers.com/understanding-bootstrap-confidence-interval-output-from-the-r-boot-package/).
...
The population is lognormal and is generated outside the simulation loop by using `pop <- rlnorm(1000000)`. This approach works well, as in our example the population mean is `r pop.m`, which is very close to the [known true population mean](https://en.wikipedia.org/wiki/Log-normal_distribution) of `r exp(1/2)`. An alternative is to generate the random numbers directly inside the loop by using `samp <- rlnorm(nsamp)`. The first approach used here is useful when there is no formula available to define the theoretical population values for certain distributions, such as the 20% trimmed mean of a lognormal distribution.
...
The `quantile()` function offers 9 types of calculations - the default is `type=7`. Hesterberg (2015) recommends using `type=6`. With sample sizes and `nboot` sufficiently large, the `type` argument will not matter, but it is unclear when this is the case.
...
The same can be done with the `boot()` function but it gets more complicated to program. We can use a helper function from the `simpleboot` package to ease the process:
```{r}
# bootstrap data
set.seed(1)
...
In the code, the variable `boot.id` is a vector of participant indices, sampled with replacement from the full list of participants. These indices are then used to index the matched vectors of `x1` and `y1`. That way, paired observations are kept together. Spearman’s correlation is computed for each of these bootstrap pairs. 
